        ...
        if (isTakingPicture && !isRunning) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    final byte[] I420src = new byte[mCameraUtil.getCameraWidth() * mCameraUtil.getCameraHeight() * 3 / 2];
                    YuvUtil.NV21ToI420(srcData, mCameraUtil.getCameraWidth(), mCameraUtil.getCameraHeight(), I420src);

                    //进行yuv数据裁剪的操作
                    final byte[] cropData = new byte[cropWidth * cropHeight * 3 / 2];
                    YuvUtil.cropYUV(I420src, mCameraUtil.getCameraWidth(), mCameraUtil.getCameraHeight(), cropData, cropWidth, cropHeight, cropStartX, cropStartY);

                    //这里将yuvi420转化为nv21，因为yuvimage只能操作nv21和yv12，为了演示方便，这里做一步转化的操作
                    final byte[] nv21Data = new byte[cropData.length];
